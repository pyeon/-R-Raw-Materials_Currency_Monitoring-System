...
from config import ALERT_THRESHOLDS, CORRELATION_PATTERNS, get_enabled_assets, REPORT_DIR
import os
...
class AlertManager:
    ...
    def _generate_markdown_report(self):
        ...
        report.append(f"# 📊 원자재/통화 시장 분석 리포트")
        report.append(f"\n**생성일시**: {now.strftime('%Y년 %m월 %d일 %H:%M:%S')}\n")
        report.append("---\n")
        
        # 종합 요약
        report.append("## 📈 종합 요약\n")
        if self.alerts['level1']:
            for line in self.alerts['level1']:
                report.append(f"- {line}")
        report.append("")
        
        # 주의 알림
        if self.alerts['level2']:
            report.append("## ⚠️ 주의 알림\n")
            for line in self.alerts['level2']:
                report.append(f"- {line}")
            report.append("")
        
        # 긴급 알림
        if self.alerts['level3']:
            report.append("## 🚨 긴급 알림\n")
            for line in self.alerts['level3']:
                report.append(f"- {line}")
            report.append("")
        ...
        for category, assets in self.assets.items():
            for code, info in assets.items():
                ...
                d = self.data[code]
                report.append(f"### {info['icon']} {info['name']}\n")
                report.append(f"- **현재가**: {d['current_price']:,.2f}")
                report.append(f"- **전일대비**: {d['daily_change']:+,.2f} ({d['daily_change_pct']:+.2f}%)")
                
                # 이동평균
                if 'moving_averages' in d:
                    ma = d['moving_averages']
                    if 'MA5' in ma:
                        report.append(f"- **MA5**: {ma['MA5']['value']:,.2f} (괴리율: {ma['MA5']['divergence']:+.2f}%)")
                    if 'MA20' in ma:
                        report.append(f"- **MA20**: {ma['MA20']['value']:,.2f} (괴리율: {ma['MA20']['divergence']:+.2f}%)")
                
                # 52주 고저
                report.append(f"- **52주 최고**: {d.get('52w_high', 0):,.2f}")
                report.append(f"- **52주 최저**: {d.get('52w_low', 0):,.2f}")
                
                report.append("")
        
        # 상관관계 분석
        if 'correlations' in self.data and self.data['correlations']:
            report.append("## 🔗 상관관계 분석\n")
            correlations = self.data['correlations']
            for pair, corr in sorted(correlations.items(), key=lambda x: abs(x[1]), reverse=True)[:10]:
                codes = pair.split('_')
                names = [self._get_asset_name(c) for c in codes]
                pair_name = ' vs '.join(names)
                report.append(f"- **{pair_name}**: {corr:.3f}")
            report.append("")
        
        report.append("---")
        report.append(f"\n*Generated by Commodity Monitor System v1.0*")
        
        # 파일 저장
        filename = f"{REPORT_DIR}/market_analysis_{now.strftime('%Y%m%d_%H%M%S')}.md"
        with open(filename, 'w', encoding='utf-8') as f:
            f.write('\n'.join(report))
        
        print(f"✅ 마크다운 리포트 생성: {filename}")
