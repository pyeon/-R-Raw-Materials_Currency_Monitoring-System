...
from config import MOVING_AVERAGES, LOOKBACK_PERIODS, DATA_DIR
...
class DataProcessor:
    ...
    def _process_single_asset(self, code: str, df: pd.DataFrame) -> Dict:
        ...
        # 최근 7일 데이터
        last_7days = df['close'].tail(7)
        result['last_7days'] = {
            str(idx.date()): float(val) 
            for idx, val in last_7days.items()
        }
    ...
    def _save_processed_data(self):
        ...
        if 'correlations' in self.results:
            json_data['correlations'] = self.results['correlations']
        
        # JSON 파일로 저장
        filepath = f"{DATA_DIR}/processed_data.json"
        with open(filepath, 'w', encoding='utf-8') as f:
            json.dump(json_data, f, ensure_ascii=False, indent=2)
        print(f"✅ 처리 데이터 저장: {filepath}")
